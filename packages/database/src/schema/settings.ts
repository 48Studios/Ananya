...
export const organizationProfile = pgTable(
  "organization_profile",
  {
    ...
    companyName: varchar("company_name", { length: 255 }).notNull().default("48 Studios"),
    legalName: varchar("legal_name", { length: 255 }).notNull().default("48 Studios Pvt Ltd"),
    registrationNumber: varchar("registration_number", { length: 128 }),
    taxId: varchar("tax_id", { length: 128 }).notNull().default("GSTIN-33AAACD4848A1Z5"),
    email: varchar("email", { length: 255 }).notNull().default("ops@48studios.com"),
    ...
    primaryTimezone: varchar("primary_timezone", { length: 64 }).default("Asia/Kolkata"),
...
export const systemSettings = pgTable(
  "system_settings",
  {
    ...
    supportedCurrencies: jsonb("supported_currencies").$type<string[]>().default(["INR", "USD", "EUR"]),
    ...
    reorderDefaultsJson: jsonb("reorder_defaults_json").$type<Record<string, number>>().default({
      minStockLevel: 10,
      reorderQuantity: 50,
    }),
    taxRatesJson: jsonb("tax_rates_json").$type<Array<{ name: string; rate: number }>>().default([
      { name: "GST 18%", rate: 18 },
      { name: "GST 12%", rate: 12 },
      { name: "GST 5%", rate: 5 },
    ]),
...
export const featureFlags = pgTable(
  "feature_flags",
  {
    ...
    category: varchar("category", { length: 64 }).notNull().default("EXPERIMENTAL"),
