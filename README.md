...
├── compose.yaml              # Primary Docker Compose stack definition
├── compose.prod.yaml         # Hardened production Compose definition
├── compose.dev.yaml          # Local development Compose definition (with pgAdmin)
...
| **Cache & Queue**      | Redis              | `7.x`   | High-speed cache and queue processing         |
...
### 3. Initialize Database & System Bootstrap

Start PostgreSQL & Redis (via development compose), then run system setup:

```bash
# Launch development datastores
...
### Production Docker Compose Quick Start

```bash
# 1. Pull latest production images from GHCR
docker compose -f compose.prod.yaml pull

# 2. Start full production stack (Web, API, Worker, PostgreSQL, Redis)
...
| `REDIS_URL`           | Yes      | `redis://localhost:6379`                                           | Connection string for Redis cache & queue store            |
...
- Datastores (PostgreSQL, Redis) are isolated on private internal container networks.
