...
Build and run the exact production images from source without pushing to GHCR:
...
```bash
# Inspect the fully-resolved local stack
docker compose -f compose.yml -f compose.local.yml config

# Inspect the fully-resolved production stack
...
# 2. Start full production stack (Web, API, PostgreSQL)
docker compose -f compose.yml -f compose.prod.yml up -d

# 3. Optionally enable the Worker and pgAdmin profiles
docker compose -f compose.yml -f compose.prod.yml --profile worker --profile tools up -d
```
...
ANANYA_VERSION=0.1.0 docker compose -f compose.yml -f compose.prod.yml up -d

# Pin to a specific git commit SHA
ANANYA_VERSION=sha-a1b2c3d docker compose -f compose.yml -f compose.prod.yml up -d
```
...
> [!NOTE]
> **Volume Migration Note**: If you previously used the old standalone `compose.prod.yml`, volumes were named `ananya_postgres_data` (same). If upgrading from the old `compose.yml` dev stack, run `docker volume prune` to remove the legacy `ananya_dev_*` volumes.
