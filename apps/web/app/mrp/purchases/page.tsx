...
export default function PurchaseRecommendationsPage() {
  ...
  const fetchRecommendations = useCallback(async () => {
    try {
      const data = await api.getPurchaseRecommendations();
  ...
  const handleAccept = async (id: string) => {
    try {
      await api.acceptPurchaseRecommendation(id);
      fetchRecommendations();
    } catch (err: unknown) {
  ...
  const handleReject = async (id: string) => {
    try {
      await api.rejectPurchaseRecommendation(id);
      fetchRecommendations();
    } catch (err: unknown) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center border-b pb-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight">Purchase Planning Recommendations</h1>
          <p className="text-sm text-gray-500">MRP component order recommendations, suggested quantities, and procurement handoff.</p>
        </div>
      </div>
          ...
          <tbody className="divide-y">
            {recommendations.map((rec) => (
              <tr key={String(rec.id)} className="hover:bg-gray-50">
                <td className="p-3 font-mono font-bold text-gray-900">{String(rec.componentId)}</td>
                <td className="p-3 font-mono font-bold text-blue-700">{Number(rec.suggestedQuantity).toFixed(2)}</td>
                <td className="p-3 font-mono text-gray-700">{new Date(String(rec.requiredDate)).toLocaleDateString()}</td>
                <td className="p-3 text-gray-700 max-w-xs">{String(rec.recommendationReason)}</td>
                <td className="p-3">
            ...
            {recommendations.length === 0 && (
              <tr>
                <td colSpan={6} className="p-4 text-center text-gray-500">No purchase recommendations found.</td>
              </tr>
            )}
          </tbody>
        </table>
