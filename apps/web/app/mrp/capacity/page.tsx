...
export default function MrpCapacityPage() {
  ...
  const averageUtilization = React.useMemo(() => {
    ...
    const sum = centers.reduce((acc, c) => acc + (c?.utilizationPercentage || 0), 0)
  ...
  const columns: ColumnDef<WorkCenterCapacityDto>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <span className="font-mono text-xs text-muted-foreground">{row.original.availableHoursWeekly || 0} hrs</span>,
    },
    {
      accessorKey: 'allocatedHoursWeekly',
      header: 'Scheduled Load',
      cell: ({ row }) => <span className="font-mono text-xs text-foreground font-semibold">{row.original.allocatedHoursWeekly || 0} hrs</span>,
    },
    {
