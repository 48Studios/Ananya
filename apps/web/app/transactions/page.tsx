...
export default function TransactionsPage() {
  ...
  const receiptCount = React.useMemo(
    () => transactions.filter((t) => ['Receipt', 'InitialStock'].includes(t.transactionType)).length,
    [transactions],
  )
  ...
  const issueCount = React.useMemo(
    () => transactions.filter((t) => ['Issue', 'Consumption', 'Adjustment'].includes(t.transactionType)).length,
    [transactions],
  )
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const sign = ['Issue', 'Consumption'].includes(row.original.transactionType) ? '-' : '+'
          return (
            <span className="font-mono text-xs font-bold text-foreground">
              {sign}{type} {row.original.unitOfMeasure}
            </span>
          )
        },
      },
      {
        ...
        header: 'Location',
        cell: ({ row }) => {
          const src = row.original.sourceLocationId ? locationsMap[row.original.sourceLocationId] : null
          const dest = row.original.destinationLocationId ? locationsMap[row.original.destinationLocationId] : null
      ...
      },
      {
        ...
        header: 'Performed By',
        cell: ({ row }) => (
          <span className="text-xs text-foreground">{row.original.createdBy}</span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center justify-end gap-1">
            <Link href={`/transactions/${row.original.id}`}>
              <Button variant="ghost" size="icon-xs" title="View transaction details">
