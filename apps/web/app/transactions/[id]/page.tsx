...
export default function ViewTransactionPage() {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      if (
        txData.destinationLocationId &&
        locMap[txData.destinationLocationId]
      ) {
  ...
  if (loading) {
    return (
      <LoadingState message="Loading inventory transaction audit entry..." />
    );
  }
  ...
  if (error || !tx) {
    return (
      <ErrorState
        ...
        message={
          error || "The requested inventory transaction record does not exist."
        }
        onRetry={fetchData}
      />
    );
          ...
          <div className="space-y-3 text-xs">
            <div className="p-3 bg-muted/40 border border-border rounded-lg space-y-1 text-muted-foreground">
              ...
              <p className="text-[11px]">
                Inventory transactions represent historical facts in the system
                ledger. They cannot be edited or deleted under any
                circumstances.
              </p>
            </div>
