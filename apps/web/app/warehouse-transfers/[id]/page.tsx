...
export default function ViewWarehouseTransferPage() {
  ...
  const [transfer, setTransfer] = React.useState<WarehouseTransferDto | null>(null)
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      const transferTxs = txs.filter((t) => t.reference === data.transferNumber)
  ...
  if (error || !transfer) {
    return (
      <ErrorState
        ...
        message={error || 'The requested Warehouse Transfer record does not exist.'}
        onRetry={fetchData}
      />
    )
  }
    ...
    <div className="space-y-6 print:space-y-4">
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2 print:hidden">
            <Button variant="outline" size="sm" onClick={() => router.push('/warehouse-transfers')}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-2xl bg-card border border-border rounded-xl shadow-lg p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-semibold text-foreground">Edit Draft Warehouse Transfer</h2>
              <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-card border border-border rounded-xl p-6 space-y-6 shadow-xs">
          <div className="flex items-center justify-between border-b border-border pb-4">
            <div>
              <h3 className="text-base font-semibold text-foreground">Warehouse Transfer Details</h3>
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Transfer Number</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Source Dispatch Location</dt>
              <dd className="mt-1 font-medium text-foreground">
                {sourceLocation ? (
                  <Link href={`/locations/${sourceLocation.id}`} className="hover:underline flex items-center gap-1">
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Destination Receiving Location</dt>
              <dd className="mt-1 font-medium text-foreground">
                {destLocation ? (
                  <Link href={`/locations/${destLocation.id}`} className="hover:underline flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-muted-foreground" />
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Requested Date</dt>
              <dd className="mt-1 text-sm text-foreground flex items-center gap-1 font-mono">
                <Calendar className="w-3.5 h-3.5 text-muted-foreground" />
                {transfer.requestedDate ? new Date(transfer.requestedDate).toLocaleDateString() : 'Not set'}
              </dd>
            </div>
            ...
            <div>
              ...
              <dd className="mt-1 text-sm text-foreground flex items-center gap-1 font-mono">
                ...
                {transfer.dispatchedAt ? new Date(transfer.dispatchedAt).toLocaleString() : 'In transit / pending'}
              </dd>
            </div>

            <div>
              <dt className="text-xs font-medium text-muted-foreground">Receipt Completion Date</dt>
              <dd className="mt-1 text-sm text-foreground flex items-center gap-1 font-mono">
                <CheckCircle2 className="w-3.5 h-3.5 text-muted-foreground" />
                {transfer.receivedAt ? new Date(transfer.receivedAt).toLocaleString() : 'Pending receipt'}
              </dd>
            </div>
          </dl>
          ...
          {transfer.notes && (
            <div className="pt-4 border-t border-border space-y-1">
              <span className="text-xs font-medium text-muted-foreground">Transfer Instructions / Notes</span>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
          <h3 className="text-base font-semibold text-foreground">Lifecycle Stage</h3>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-foreground">Draft Created</span>
                <p className="text-[11px] text-muted-foreground">Line items prepared</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                  ['SUBMITTED', 'DISPATCHED', 'RECEIVED'].includes(transfer.status)
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] text-muted-foreground">Pending stock dispatch</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-foreground">Dispatched (In Transit)</span>
                <p className="text-[11px] text-muted-foreground">Outbound ledger posted</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-foreground">Received & Completed</span>
                <p className="text-[11px] text-muted-foreground">Inbound ledger verified</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-semibold text-foreground">
            Inventory Ledger Audit Log ({linkedTransactions.length} Posted Entries)
          </h3>
        ...
        {linkedTransactions.length === 0 ? (
          <p className="text-xs text-muted-foreground p-4 bg-muted/20 border border-border rounded-lg text-center">
            No inventory transactions posted yet. Outbound transactions post on dispatch, and inbound transactions post on receipt.
          </p>
        ) : (
          <div className="overflow-x-auto border border-border rounded-lg">
            <table className="w-full text-left text-xs">
