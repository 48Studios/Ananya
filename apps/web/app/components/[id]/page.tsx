...
import { Edit3, Trash2, ArrowLeft, Package, Layers, MapPin, Activity, CheckCircle2 } from 'lucide-react'
...
export default function ViewComponentPage() {
  ...
  const [defaultLocation, setDefaultLocation] = React.useState<LocationDto | null>(null)
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => router.push('/components')}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <StatCard title="Current Stock" value="0" subtitle={`Unit: ${component.unit}`} icon={Package} />
        <StatCard title="Reserved" value="0" subtitle="Allocated orders" icon={Layers} />
        <StatCard title="Available" value="0" subtitle="Net downloadable" icon={Package} />
        <StatCard title="Reorder Level" value="10" subtitle="Safety threshold" icon={Activity} />
        <StatCard title="Min Stock" value="5" subtitle="Floor balance" icon={Activity} />
        <StatCard title="Max Stock" value="100" subtitle="Ceiling limit" icon={Activity} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-card border border-border rounded-xl p-6 space-y-6 shadow-xs">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Component ID</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">SKU / Part Number</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Default Unit</dt>
            ...
            <div>
              ...
              <dd className="mt-1 text-xs text-muted-foreground">
                {component.manufacturerId ? component.manufacturerId : 'Unassigned'}
              </dd>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-semibold text-foreground">Default Storage</h3>
            ...
            {defaultLocation ? (
              <div className="space-y-2 p-3 bg-muted/30 border border-border rounded-lg">
                ...
                <p className="text-sm font-medium text-foreground">{defaultLocation.name}</p>
                <Link href={`/locations/${defaultLocation.id}`}>
                  <Button variant="link" size="xs" className="px-0 text-primary">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md bg-card border border-border rounded-xl shadow-lg p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-semibold text-foreground">Edit Component</h2>
            </div>
            <ComponentForm
