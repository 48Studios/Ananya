...
import { Edit3, Trash2, ArrowLeft, Factory, Package, Calendar, CheckCircle2 } from 'lucide-react'
...
export default function ViewManufacturerPage() {
  ...
  const [manufacturer, setManufacturer] = React.useState<ManufacturerDto | null>(null)
    ...
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => router.push('/manufacturers')}>
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-6 shadow-xs">
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
          <div>
            <dt className="text-xs font-medium text-muted-foreground">Manufacturer ID</dt>
          ...
          <div>
            ...
            <dd className="mt-1 text-sm font-medium text-foreground">{manufacturer.name}</dd>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md bg-card border border-border rounded-xl shadow-lg p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-semibold text-foreground">Edit Manufacturer</h2>
            </div>
            <ManufacturerForm
              ...
              onSuccess={(updated) => {
                ...
                setIsEditOpen(false)
                setToastMessage(`Manufacturer "${updated.code}" updated successfully.`)
