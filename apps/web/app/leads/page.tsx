...
const mockLeads: LeadRecord[] = [
  { id: 'ld-1', leadNumber: 'LD-2026-01', contactName: 'Sarah Jenkins', companyName: 'Nexus Defense Industries', email: 's.jenkins@nexusdefense.com', status: 'QUALIFIED', createdDate: '2026-02-01' },
  { id: 'ld-2', leadNumber: 'LD-2026-02', contactName: 'David Chen', companyName: 'Precision Robotics Lab', email: 'd.chen@precisionrobotics.io', status: 'NEW', createdDate: '2026-02-03' },
]
  ...
  const columns: ColumnDef<LeadRecord>[] = [
    {
      ...
      header: 'Lead ID',
      cell: ({ row }) => (
        <Link href={`/leads/${row.original.id}`} className="font-mono text-xs font-bold text-primary hover:underline">
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">{row.original.contactName}</p>
        </div>
      ),
    },
    {
