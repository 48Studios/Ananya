...
export default function ManufacturingReportsPage() {
  const [summary, setSummary] = React.useState<ManufacturingSummaryDto | null>(
    null,
  );
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to load manufacturing report data",
      );
    } finally {
      setLoading(false);
    }
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-mono text-xs font-bold text-foreground">
            {row.original.quantityCompleted} / {row.original.quantityPlanned}{" "}
            units
          </span>
        ),
      },
