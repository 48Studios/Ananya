...
export default function InventoryReportsPage() {
  const [summary, setSummary] = React.useState<InventorySummaryDto | null>(
    null,
  );
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to load inventory reporting data",
      );
    } finally {
      setLoading(false);
    }
  ...
  const statusDonutData = [
    {
      name: "Active Items",
      value: summary.activeComponents ?? 0,
      color: "#10b981",
    },
    {
      name: "Inactive Items",
      value: Math.max(
        0,
        (summary.totalComponents ?? 0) - (summary.activeComponents ?? 0),
      ),
      color: "#64748b",
    },
  ];
