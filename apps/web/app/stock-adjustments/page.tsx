...
export default function StockAdjustmentsPage() {
  ...
  const columns = React.useMemo<ColumnDef<StockAdjustmentDto>[]>(
    () => [
      ...
      },
      {
        accessorKey: 'reason',
        header: 'Reason',
        cell: ({ row }) => <span className="text-xs text-foreground">{row.original.reason}</span>,
      },
      {
        accessorKey: 'lines',
      ...
      },
      {
        ...
        header: 'Created By',
        cell: ({ row }) => <span className="text-xs text-foreground">{row.original.createdBy}</span>,
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center justify-end gap-1">
            <Link href={`/stock-adjustments/${row.original.id}`}>
              <Button variant="ghost" size="icon-xs" title="View details & approval">
  ...
  const handleFormSuccess = (savedAdj: StockAdjustmentDto) => {
    ...
    setToastMessage(`Stock Adjustment "${savedAdj.adjustmentNumber}" submitted for approval.`)
    setIsFormOpen(false)
