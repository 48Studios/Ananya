...
export default function ViewStockAdjustmentPage() {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to cancel adjustment",
      );
    } finally {
  ...
  if (error || !adj) {
    return (
      <ErrorState
        ...
        message={
          error || "The requested Stock Adjustment record does not exist."
        }
        onRetry={fetchData}
      />
    );
            ...
            {adj.approvedBy && (
              <div>
                ...
                <dd className="mt-1 text-sm font-semibold text-emerald-600 dark:text-emerald-400">
                  {adj.approvedBy} on{" "}
                  {adj.approvedAt
                    ? new Date(adj.approvedAt).toLocaleString()
                    : "—"}
                </dd>
              </div>
            )}
            ...
            {adj.status === "APPROVED" ? (
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-700 dark:text-emerald-400 space-y-1">
                ...
                <p className="text-[11px] opacity-90">
                  `Adjustment` inventory transactions recorded. Stock
                  projections updated at location.
                </p>
              </div>
            ) : (
