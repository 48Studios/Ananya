...
export default function ViewProjectPage() {
  ...
  const handleAllocateMaterial = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMaterialError(err instanceof Error ? err.message : 'Allocation failed')
    } finally {
  ...
  const totalAllocated = project.materials.reduce((s, m) => s + m.allocatedQuantity, 0)
  const totalIssued = project.materials.reduce((s, m) => s + m.issuedQuantity, 0)
      ...
      <div className="w-full max-w-lg bg-card border border-border rounded-xl shadow-lg p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-border">
            <Button type="button" variant="outline" size="sm" onClick={resetMaterialForm}>
              Cancel
            </Button>
  ...
  return (
    <div className="space-y-6 print:space-y-4">
      ...
      {/* Material Forms */}
      {showAllocateForm && renderMaterialForm('Allocate Material', handleAllocateMaterial, 'Allocate', true)}
      {showIssueForm && renderMaterialForm('Issue Material', handleIssueMaterial, 'Issue Material')}
      {showReturnForm && renderMaterialForm('Return Material', handleReturnMaterial, 'Return Material')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-card border border-border rounded-xl p-6 space-y-6 shadow-xs">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Project Number</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Project Manager</dt>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
          ...
          <div className="space-y-4 pt-2">
            {(['PLANNING', 'ACTIVE', 'ON_HOLD', 'COMPLETED'] as const).map((stage, idx) => {
              const labels: Record<string, { label: string; desc: string }> = {
                PLANNING: { label: 'Planning', desc: 'Project created, allocations open' },
                ACTIVE: { label: 'Active', desc: 'Production in progress' },
                ON_HOLD: { label: 'On Hold', desc: 'Temporarily paused' },
                COMPLETED: { label: 'Completed', desc: 'Deliverables accepted' },
              }
              const stageOrder = ['PLANNING', 'ACTIVE', 'ON_HOLD', 'COMPLETED'] as const
              const currentIdx = stageOrder.indexOf(project.status as typeof stageOrder[number])
              const isReached =
                currentIdx >= idx ||
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 print:hidden">
            {canAllocate && (
              <Button size="xs" variant="outline" onClick={() => setShowAllocateForm(true)}>
            ...
            {canIssueReturn && (
              <>
                <Button size="xs" variant="outline" onClick={() => setShowIssueForm(true)}>
                  Issue
                </Button>
                <Button size="xs" variant="outline" onClick={() => setShowReturnForm(true)}>
                  Return
                </Button>
              </>
            )}
        ...
        {project.materials.length === 0 ? (
          <p className="text-xs text-muted-foreground p-4 bg-muted/20 border border-border rounded-lg text-center">
            No materials allocated to this project. Use &quot;Allocate&quot; to reserve components.
          </p>
        ) : (
                    ...
                    <tr key={mat.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="p-3 font-medium">
                        {comp ? (
                          ...
                          <span className="font-mono">{mat.componentId.slice(0, 8)}</span>
                        )}
                      </td>
                      <td className="p-3">
                        {loc ? (
                          ...
                          <span className="font-mono text-xs">{mat.locationId.slice(0, 8)}</span>
                        )}
                      </td>
                      ...
                      <td className="p-3 font-mono text-muted-foreground">{mat.unitOfMeasure}</td>
                    </tr>
                  )
                })}
              </tbody>
            ...
            {project.activities.map((act) => (
              ...
              >
                ...
                <div className="flex-1 space-y-0.5">
                  ...
                  <p className="text-[11px] text-muted-foreground">By: {act.performedBy}</p>
                </div>
              </div>
            ))}
          </div>
