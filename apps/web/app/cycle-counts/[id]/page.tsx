...
export default function ViewCycleCountPage() {
  ...
  const [cycleCount, setCycleCount] = React.useState<CycleCountDto | null>(null)
  const [summary, setSummary] = React.useState<DiscrepancySummaryDto | null>(null)
  ...
  const handleApprove = async () => {
    ...
    setIsApproving(true)
    try {
      const updated = await cycleCountsApi.approve(cycleCount.id, 'AUDIT_MANAGER')
      setCycleCount(updated)
      setShowApproveDialog(false)
      fetchData()
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to approve cycle count')
    } finally {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to cancel cycle count')
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete cycle count')
    } finally {
      setIsDeleting(false)
    }
  }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2 print:hidden">
            <Button variant="outline" size="sm" onClick={() => router.push('/cycle-counts')}>
            ...
            {cycleCount.status === 'DRAFT' && (
              <>
                ...
                <Button size="sm" onClick={() => handleStartCounting()} disabled={isStarting}>
            ...
            {(cycleCount.status === 'ASSIGNED' || cycleCount.status === 'COUNTING') && (
              <>
            ...
            {cycleCount.status === 'REVIEW' && (
              <>
                <Button variant="outline" size="sm" onClick={() => setIsRecordModalOpen(true)}>
                  <Pencil className="w-4 h-4 mr-1.5" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-2xl bg-card border border-border rounded-xl shadow-lg p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-semibold text-foreground">Edit Draft Cycle Count</h2>
              <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-card border border-border rounded-xl p-6 space-y-6 shadow-xs">
          <div className="flex items-center justify-between border-b border-border pb-4">
            <div>
              <h3 className="text-base font-semibold text-foreground">Cycle Count Specification</h3>
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Count Number</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Facility / Location</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Assigned Counter Specialist</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Scheduled Count Date</dt>
              <dd className="mt-1 text-sm text-foreground flex items-center gap-1 font-mono">
                <Calendar className="w-3.5 h-3.5 text-muted-foreground" />
                {cycleCount.scheduledDate ? new Date(cycleCount.scheduledDate).toLocaleDateString() : 'Not set'}
              </dd>
            </div>

            <div>
              <dt className="text-xs font-medium text-muted-foreground">Count Execution Date</dt>
              <dd className="mt-1 text-sm text-foreground flex items-center gap-1 font-mono">
                <Clock className="w-3.5 h-3.5 text-muted-foreground" />
                {cycleCount.completedAt ? new Date(cycleCount.completedAt).toLocaleString() : 'Pending physical count'}
              </dd>
            </div>

            <div>
              <dt className="text-xs font-medium text-muted-foreground">Reconciliation Approval Date</dt>
              <dd className="mt-1 text-sm text-foreground flex items-center gap-1 font-mono">
                <CheckCircle2 className="w-3.5 h-3.5 text-muted-foreground" />
                {cycleCount.approvedAt ? new Date(cycleCount.approvedAt).toLocaleString() : 'Pending manager review'}
              </dd>
            </div>
          </dl>
          ...
          {cycleCount.notes && (
            <div className="pt-4 border-t border-border space-y-1">
              <span className="text-xs font-medium text-muted-foreground">Audit Notes & Scope</span>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
          <h3 className="text-base font-semibold text-foreground">Audit Workflow</h3>
          ...
          <div className="space-y-4 pt-2">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-foreground">Schedule & Scope</span>
                <p className="text-[11px] text-muted-foreground">Target components assigned</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-foreground">Physical Floor Count</span>
                <p className="text-[11px] text-muted-foreground">Actual floor stock entered</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-foreground">Variance Review</span>
                <p className="text-[11px] text-muted-foreground">Inspect Shortage/Surplus</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-foreground">Reconcile & Ledger</span>
                <p className="text-[11px] text-muted-foreground">Generate Stock Adjustment</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-semibold text-foreground">
            Cycle Count Lines & Variance Analysis ({cycleCount.lines.length} Items)
          </h3>
                  ...
                  <tr key={line.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 text-right font-mono font-bold">
                      {line.variance === 0 ? (
                        ...
                        <span className="text-rose-600 dark:text-rose-400">{line.variance}</span>
                      ) : (
                        <span className="text-blue-600 dark:text-blue-400">+{line.variance}</span>
                      )}
                    </td>
