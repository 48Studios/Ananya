...
import { inventoryTransactionsApi, type InventoryTransactionDto } from '@/lib/api/inventory-transactions-api'
...
export default function ViewWorkOrderPage() {
  ...
  const [productComp, setProductComp] = React.useState<ComponentDto | null>(null)
  ...
  const [materials, setMaterials] = React.useState<MaterialRequirementDetailDto[]>([])
  ...
  const [linkedTransactions, setLinkedTransactions] = React.useState<InventoryTransactionDto[]>([])
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      const [pComp, bomData, locData, matReqs, activityItems, comps, txs] = await Promise.all([
        componentsApi.getById(woData.componentId).catch(() => null),
        bomsApi.getById(woData.bomId).catch(() => null),
        woData.locationId ? locationsApi.getById(woData.locationId).catch(() => null) : Promise.resolve(null),
  ...
  const handlePause = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to pause production')
    } finally {
      setIsPausing(false)
    }
  }
  ...
  const handleResume = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to resume production')
    } finally {
      setIsResuming(false)
    }
  }
    ...
    <div className="space-y-6 print:space-y-4">
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2 print:hidden">
            ...
            {(wo.status === 'RELEASED' || wo.status === 'MATERIAL_ALLOCATED') && (
              <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-2xl bg-card border border-border rounded-xl shadow-lg p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-semibold text-foreground">Edit Draft Work Order</h2>
              <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 print:hidden">
        ...
        <StatCard
          ...
          subtitle={remainingUnits === 0 ? 'Production complete' : 'To be manufactured'}
          icon={Package}
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-card border border-border rounded-xl p-6 space-y-6 shadow-xs">
          <div className="flex items-center justify-between border-b border-border pb-4">
            <div>
              <h3 className="text-base font-semibold text-foreground">Work Order Summary</h3>
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Work Order #</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Finished Product</dt>
              <dd className="mt-1 font-medium text-foreground">
                {productComp ? (
                  <Link href={`/components/${productComp.id}`} className="hover:underline flex items-center gap-1">
                    <Wrench className="w-3.5 h-3.5 text-muted-foreground" />
                    {productComp.name} <span className="font-mono text-xs text-muted-foreground">({productComp.sku})</span>
                  </Link>
                ) : (
                  wo.componentId
                )}
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">BOM Revision</dt>
              <dd className="mt-1 font-medium text-foreground">
                {bom ? (
                  <Link href={`/boms/${bom.id}`} className="hover:underline font-mono text-xs text-primary">
                    BOM {bom.revision}
                  </Link>
                ) : (
                  wo.bomId
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Production Location</dt>
            ...
            <div>
              <dt className="text-xs font-medium text-muted-foreground">Planned Start Date</dt>
              <dd className="mt-1 text-sm text-foreground flex items-center gap-1 font-mono">
                <Calendar className="w-3.5 h-3.5 text-muted-foreground" />
                {wo.startDate ? new Date(wo.startDate).toLocaleDateString() : 'Not scheduled'}
              </dd>
            </div>

            <div>
              <dt className="text-xs font-medium text-muted-foreground">Planned Completion Date</dt>
              <dd className="mt-1 text-sm text-foreground flex items-center gap-1 font-mono">
                <Calendar className="w-3.5 h-3.5 text-muted-foreground" />
                {wo.endDate ? new Date(wo.endDate).toLocaleDateString() : 'Not scheduled'}
              </dd>
            </div>
          </dl>
          ...
          {wo.notes && (
            <div className="pt-4 border-t border-border space-y-1">
              <span className="text-xs font-medium text-muted-foreground">Production Notes</span>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
          <h3 className="text-base font-semibold text-foreground">Production Progress</h3>

          <div className="space-y-2">
            <div className="flex justify-between text-xs font-medium">
              <span className="text-muted-foreground font-semibold">Yield Completion</span>
          ...
          <div className="space-y-2 pt-2 text-xs">
            <div className="flex justify-between p-2.5 bg-muted/30 border border-border rounded-lg">
              ...
              <span className="font-mono font-bold text-foreground">{wo.quantityPlanned}</span>
            </div>
            <div className="flex justify-between p-2.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-700 dark:text-emerald-400">
              ...
              <span className="font-mono font-bold">{wo.quantityCompleted}</span>
            </div>
            <div className="flex justify-between p-2.5 bg-muted/30 border border-border rounded-lg">
              <span className="text-muted-foreground">Units Remaining:</span>
              <span className="font-mono font-bold text-foreground">{remainingUnits}</span>
            </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-semibold text-foreground">
            Material Requirements & Consumption ({materials.length} Raw Materials)
          </h3>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-border">
              {materials.map((mat, idx) => {
                ...
                return (
                  <tr key={mat.componentId} className="hover:bg-muted/30 transition-colors">
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
        ...
        {timeline.length === 0 ? (
          <p className="text-xs text-muted-foreground p-4 bg-muted/20 border border-border rounded-lg text-center">
            No production execution activity recorded yet. Start production or record output batches to populate the timeline.
          </p>
        ) : (
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-semibold text-foreground">
            Inventory Ledger Transactions ({linkedTransactions.length} Posted Entries)
          </h3>
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="p-3 text-right font-mono font-bold">
                        {tx.sourceLocationId ? (
                          <span className="text-rose-600 dark:text-rose-400">-{tx.quantity}</span>
                        ) : (
                          <span className="text-emerald-600 dark:text-emerald-400">+{tx.quantity}</span>
                        )}{' '}
                        {tx.unitOfMeasure}
                      </td>
