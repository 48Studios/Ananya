...
import { maintenanceApi, type MaintenanceScheduleDto } from '@/lib/api/maintenance-api'
...
export default function MaintenancePage() {
  ...
  const handleTogglePause = async (schedule: MaintenanceScheduleDto) => {
    try {
      ...
      setToastMessage(`Schedule ${schedule.status === 'PAUSED' ? 'resumed' : 'paused'} successfully.`)
      setTimeout(() => setToastMessage(null), 4000)
    } catch {
  ...
  const columns: ColumnDef<MaintenanceScheduleDto>[] = [
    {
      ...
      cell: ({ row }) => (
        <div>
          <p className="font-semibold text-xs text-primary">{row.original.equipmentName || 'Asset'}</p>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-xs text-muted-foreground">
          {row.original.lastCompletedDate ? formatDate(row.original.lastCompletedDate) : '-'}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20">
          <Clock className="w-3 h-3 mr-1" /> {row.original.status || 'SCHEDULED'}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-1">
          ...
          <Button
            variant="ghost"
            size="icon-xs"
            title={row.original.status === 'PAUSED' ? 'Resume Schedule' : 'Pause Schedule'}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md space-y-4 shadow-xl">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-foreground">Schedule Equipment Maintenance</h3>
