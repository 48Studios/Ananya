...
export function SidebarFavoritesRecent({ isCollapsed }: SidebarFavoritesRecentProps) {
  const {
    pinnedItems,
    togglePinnedItem,
    recentItems,
  ...
  const getItemDetails = (href: string) => {
    ...
    return { title: title || 'Dashboard', icon: <ChevronRight className="size-3.5 text-muted-foreground" /> }
  }
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              activeTab === 'favorites' ? 'text-amber-500 font-bold' : 'text-muted-foreground'
            )}
          >
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-1 hover:text-foreground transition-colors',
              activeTab === 'recent' ? 'text-primary font-bold' : 'text-muted-foreground'
            )}
          >
      ...
      <div className="space-y-0.5 px-1">
        {itemsToDisplay.length === 0 ? (
          <div className="px-3 py-1.5 text-[11px] text-muted-foreground/70 italic">
            {activeTab === 'favorites' ? 'No favorites pinned yet.' : 'No recent visits.'}
          </div>
        ) : (
          itemsToDisplay.map((href) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Star className={cn('size-3', isPinned && 'fill-amber-500')} />
                </button>
              </div>
            )
          })
