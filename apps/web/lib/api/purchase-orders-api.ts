...
export const purchaseOrdersApi = {
  ...
  create: (payload: CreatePurchaseOrderPayload): Promise<PurchaseOrderDto> =>
    apiClient.post<PurchaseOrderDto, CreatePurchaseOrderPayload>('/purchase-orders', payload),
  update: (id: string, payload: UpdatePurchaseOrderPayload): Promise<PurchaseOrderDto> =>
    apiClient.put<PurchaseOrderDto, UpdatePurchaseOrderPayload>(`/purchase-orders/${id}`, payload),
  submit: (id: string): Promise<PurchaseOrderDto> =>
    apiClient.post<PurchaseOrderDto, Record<string, never>>(`/purchase-orders/${id}/submit`, {}),
  approve: (id: string): Promise<PurchaseOrderDto> =>
    apiClient.post<PurchaseOrderDto, Record<string, never>>(`/purchase-orders/${id}/approve`, {}),
  issue: (id: string): Promise<PurchaseOrderDto> =>
    apiClient.post<PurchaseOrderDto, Record<string, never>>(`/purchase-orders/${id}/issue`, {}),
  cancel: (id: string): Promise<PurchaseOrderDto> =>
    apiClient.post<PurchaseOrderDto, Record<string, never>>(`/purchase-orders/${id}/cancel`, {}),
  delete: (id: string): Promise<void> => apiClient.delete<void>(`/purchase-orders/${id}`),
};
