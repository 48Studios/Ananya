...
export const mrpApi = {
  ...
  getShortages: async (): Promise<MaterialShortageDto[]> => {
    return apiClient.get<MaterialShortageDto[]>('/material-requirements/shortages')
  },
  getProductionRecommendations: async (): Promise<PlannedProductionOrderDto[]> => {
    return apiClient.get<PlannedProductionOrderDto[]>('/production-recommendations')
  },
  getPurchaseRecommendations: async (): Promise<PlannedPurchaseOrderDto[]> => {
    return apiClient.get<PlannedPurchaseOrderDto[]>('/purchase-recommendations')
  },
