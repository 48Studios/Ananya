...
export const usersApi = {
  ...
  disable: (id: string): Promise<{ success: boolean }> =>
    apiClient.post<{ success: boolean }, Record<string, never>>(`/users/${id}/disable`, {}),

  activate: (id: string): Promise<{ success: boolean }> =>
    apiClient.post<{ success: boolean }, Record<string, never>>(`/users/${id}/activate`, {}),

  adminResetPassword: (id: string, newPassword: string): Promise<{ success: boolean }> =>
    apiClient.post<{ success: boolean }, { newPassword: string }>(`/users/${id}/reset-password`, { newPassword }),
};
