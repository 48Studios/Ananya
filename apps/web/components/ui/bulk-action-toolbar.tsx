...
export function BulkActionToolbar({
  ...
  const [activeAction, setActiveAction] = React.useState<BulkActionType | null>(null)
  ...
  const handleBulkExecute = async (action: BulkActionType) => {
    ...
    try {
      ...
      setMsg(`Batch action '${action}' completed for ${res.affectedCount} ${entityType} records.`)
      setTimeout(() => {
        setMsg(null)
        onClearSelection()
        onActionComplete()
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-card border border-border shadow-2xl rounded-xl px-4 py-2.5 flex items-center gap-4 text-xs animate-in slide-in-from-bottom-5 duration-200">
      ...
      {msg ? (
        <span className="text-emerald-600 dark:text-emerald-400 font-medium">{msg}</span>
      ) : (
