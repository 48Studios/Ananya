...
export function FeatureFlagTable({ flags, onFlagToggled }: FeatureFlagTableProps) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-foreground">Environment Feature Toggles</h3>
        <p className="text-xs text-muted-foreground">Manage experimental, beta, and module feature availability across environments.</p>
      </div>

      <div className="border border-border rounded-xl overflow-hidden bg-card divide-y divide-border">
        {flagList.map((flag) => (
          <div key={flag.key} className="p-4 flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">{flag.description || 'Feature flag toggle'}</p>
            </div>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  ...
                  const next = flagList.map((f) => (f.key === flag.key ? { ...f, isEnabled: val } : f))
                  setFlagList(next)
