...
import { Download, FileSpreadsheet, FileText, Check, Loader2, X } from 'lucide-react'
...
export function ExportDialog({
  ...
  const [exportScope, setExportScope] = React.useState<'ALL' | 'SELECTED'>(selectedIds.length > 0 ? 'SELECTED' : 'ALL')
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const blob = new Blob([res.fileContent], { type: 'text/plain;charset=utf-8' })
      ...
      setSuccessMsg(`Successfully exported ${res.recordCount} ${entityType} records.`)
      setTimeout(() => {
        onClose()
        setSuccessMsg(null)
      }, 1500)
      ...
      <div className="relative w-full max-w-lg bg-card border border-border rounded-xl shadow-2xl overflow-hidden p-6 space-y-5">
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-foreground">Export {entityType} Data</h2>
          </div>
        ...
        {/* Scope Selection */}
        <div className="space-y-2">
          <label className="text-xs font-semibold text-foreground">Record Scope</label>
          <div className="flex items-center gap-4 text-xs">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-foreground">All Matching Records ({totalRecordsCount})</span>
            </label>
            {selectedIds.length > 0 && (
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="text-primary focus:ring-primary"
                />
                <span className="text-foreground">Selected Rows Only ({selectedIds.length})</span>
              </label>
            )}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <label className="text-xs font-semibold text-foreground">Export Columns</label>
            <button
              type="button"
              onClick={() => setSelectedColumns(selectedColumns.length === availableColumns.length ? [] : availableColumns)}
          ...
          <div className="grid grid-cols-2 gap-2 max-h-36 overflow-y-auto p-2 bg-muted/20 border border-border rounded-lg text-xs">
            {availableColumns.map((col) => (
              <label key={col} className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="truncate text-foreground capitalize">{col}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-border pt-4">
          ...
          <Button size="sm" onClick={handleExport} disabled={loading || selectedColumns.length === 0}>
            {loading ? (
              <>
