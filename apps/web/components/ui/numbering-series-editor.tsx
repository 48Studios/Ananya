...
export function NumberingSeriesEditor({ seriesList, onSeriesUpdated }: NumberingSeriesEditorProps) {
  const [editingList, setEditingList] = React.useState<NumberingSeriesDto[]>(seriesList)
  ...
  const formatSampleCode = (item: NumberingSeriesDto) => {
    ...
    const seqStr = String(item.nextSequenceNumber).padStart(item.zeroPadLength, '0')
    return item.dateFormat === 'YYYY' ? `${item.prefix}${year}-${seqStr}` : `${item.prefix}${seqStr}`
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-foreground">Document Numbering Series</h3>
          <p className="text-xs text-muted-foreground">Configure automatic prefix and sequence rules for document codes.</p>
        </div>
      </div>
          ...
          <div key={item.id || idx} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1 min-w-[160px]">
              ...
              <div className="text-[11px] text-muted-foreground font-mono">
                Preview: <strong className="text-primary">{formatSampleCode(item)}</strong>
              </div>
            </div>

            <div className="flex items-center gap-3 flex-wrap">
              <div>
                <label className="text-[10px] text-muted-foreground block">Prefix</label>
                <input
                  type="text"
                  value={item.prefix}
                  onChange={(e) => {
              ...
              <div>
                <label className="text-[10px] text-muted-foreground block">Next Seq #</label>
                <input
                  type="number"
              ...
              <div>
                <label className="text-[10px] text-muted-foreground block">Zero Pad</label>
                <input
                  type="number"
