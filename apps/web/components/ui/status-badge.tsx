...
export function StatusBadge({ status, label, className }: StatusBadgeProps) {
  ...
  if (
    ['ACTIVE', 'COMPLETED', 'SUCCESS', 'FULFILLED', 'APPROVED', 'RESOLVED', 'CREDITED', 'DELIVERED', 'PAID'].includes(
      normalized,
    )
  ) {
    variantStyles =
    ...
    ['PENDING', 'IN_REVIEW', 'ON_HOLD', 'PAUSED', 'OVERDUE', 'SHORTAGE', 'PARTIALLY_RECEIVED'].includes(
      normalized,
    )
  ) {
    variantStyles =
  ...
  } else if (['REJECTED', 'CANCELLED', 'FAILED', 'BLOCKED', 'UNPAID'].includes(normalized)) {
    variantStyles =
      'bg-destructive/10 text-destructive dark:text-destructive border-destructive/20'
    IconComponent = XCircle
  } else if (
    ['SCHEDULED', 'SUBMITTED', 'OPEN', 'IN_PROGRESS', 'ISSUED', 'DISPATCHED'].includes(normalized)
  ) {
    variantStyles =
