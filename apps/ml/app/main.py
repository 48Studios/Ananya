...
app = FastAPI(
    ...
    description="Lightweight CPU-first machine learning service for Ananya ERP component intelligence",
...
def predict_category(req: PredictCategoryRequest):
    predictions = category_classifier.predict(req.text, top_k=req.top_k)
...
def predict_category_batch(req: BatchPredictCategoryRequest):
    results = category_classifier.predict_batch(req.texts, top_k=req.top_k)
...
def resolve_manufacturer(req: ResolveManufacturerRequest):
    return manufacturer_resolver.resolve(req.part_number, req.description or "")
...
def suggest_component(req: SuggestComponentRequest):
    ...
    cat_preds = category_classifier.predict(full_text, top_k=3)

    # 2. Manufacturer Resolution
    mfg_res = manufacturer_resolver.resolve(pn, desc)
    ...
    datasheet_res = datasheet_extractor.process(text=full_text)
