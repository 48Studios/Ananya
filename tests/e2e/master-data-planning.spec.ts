...
test.describe("Master Data & Planning Modules Completion Audit", () => {
  ...
  test("1. Warehouse Bins & Storage Locations (/warehouse-bins)", async ({ page }) => {
    await page.goto("/warehouse-bins");
    await expect(page.locator("h1")).toContainText("Warehouse Bins & Storage Locations");
  ...
  test("2. Warehouse Policies & Picking Rules (/warehouse-policies)", async ({ page }) => {
    await page.goto("/warehouse-policies");
    await expect(page.locator("h1")).toContainText("Warehouse Policies & Picking Rules");
  ...
  test("3. Physical Stock Counts (/stock-counts)", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText("Stock Audits & Cycle Counting");
  ...
  test("5. Serial Numbers (/serials)", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText("Serial Number Master Index");

    await expect(page.getByText("Total Serials Registered")).toBeVisible();

    const createBtn = page.getByRole("button", { name: "Register Serial Number" });
    await expect(createBtn).toBeVisible();
    await createBtn.click();
    await expect(page.getByRole("dialog")).toBeVisible();
  });
  ...
  test("6. Demand Projection (/projections)", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText("Financial Projections & Cash Flow Forecast");
    await expect(page.getByText("Projected Cumulative Net Inflow")).toBeVisible();
  });

  test("7. Purchase Invoices & AP Bills (/purchase-invoices)", async ({ page }) => {
    await page.goto("/purchase-invoices");
    await expect(page.locator("h1")).toContainText("Purchase Invoices & AP Bills");

    await expect(page.getByText("Total Invoices")).toBeVisible();

    const createBtn = page.getByRole("button", { name: "Enter Vendor Invoice" });
    await expect(createBtn).toBeVisible();
    await createBtn.click();
    await expect(page.getByRole("dialog")).toBeVisible();
  });

  test("8. Material Requirements Planning Engine (/mrp & subpages)", async ({ page }) => {
    await page.goto("/mrp");
    await expect(page.locator("h1")).toContainText("Material Requirements Planning (MRP) Hub");

    const runBtn = page.getByRole("button", { name: "Run MRP Calculation Engine" });
    await expect(runBtn).toBeVisible();

    await page.goto("/mrp/materials");
    await expect(page.locator("h1")).toContainText("MRP Material Shortage Matrix");

    await page.goto("/mrp/runs");
    await expect(page.locator("h1")).toContainText("MRP Execution History & Logs");

    await page.goto("/mrp/capacity");
    await expect(page.locator("h1")).toContainText("MRP Work Center Capacity Loading");

    await page.goto("/mrp/production");
    await expect(page.locator("h1")).toContainText("MRP Planned Production Orders");

    await page.goto("/mrp/purchases");
    await expect(page.locator("h1")).toContainText("MRP Planned Purchase Orders");
  });
});
